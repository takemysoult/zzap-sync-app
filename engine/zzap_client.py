...
def upload_price(cfg: ZzapConfig, file_path: str | Path, file_name: str | None = None,
    ...
    if resp.status_code == 401:
        raise RuntimeError("ZZap вернул 401 — неверный или отсутствующий API-ключ (zzap-api-key).")
    if resp.status_code == 400:
        raise RuntimeError(f"ZZap вернул 400 — неверные параметры запроса: {resp.text}")
    ...
    # Формат ответа: success / result.file_url / errors.
    if isinstance(data, dict) and data.get("success") is False:
        raise RuntimeError(f"ZZap вернул ошибку: {data.get('errors') or data}")
