...
С версии 1.2 прайс можно отправлять не только в ZZap, но и **на почту** — готовый
Excel-файл уходит письмом с вашего ящика на указанные адреса по тому же расписанию
(см. раздел «Отправка прайса на почту» ниже).
...
## Отправка прайса на почту (вместо или вместе с ZZap)

Прайс из 1С можно отправлять письмом (Excel во вложении) — например, клиенту или
менеджеру. Настройка:

1. Вкладка **«Почта»** → «Добавить…»: введите свой ящик (с него будут уходить
   ...
   **Важно:** для Mail.ru, Яндекс и Gmail нужен **«пароль приложения»** — он
   ...
   работает.
2. Нажмите «Отправить тестовое письмо…» и убедитесь, что письмо дошло.
3. На вкладке **«Ячейки»** создайте ячейку как обычно, но в поле
   **«Куда отправлять»** выберите **«На почту»**: укажите ящик-отправитель,
   получателей (можно несколько, через запятую) и, при желании, тему письма.
4. Дальше всё как с ZZap: ячейка отправляется по расписанию, при обрыве сети файл
   досылается автоматически, результаты видны в «Журнале».
